...
# # Public Key untuk browser (base64 url-safe, tanpa padding)
# public_key_b64 = base64.urlsafe_b64encode(
#     vapid.public_key.public_bytes(
#         encoding=serialization.Encoding.X962,  # Format UncompressedPoint (untuk web)
#         format=serialization.PublicFormat.UncompressedPoint
#     )
# ).rstrip(b'=').decode('utf-8')

# # Private Key untuk server (base64 url-safe, bisa dipakai oleh pywebpush)
# private_key_bytes = vapid.private_key.private_bytes(
#     encoding=serialization.Encoding.PEM,             # Format PEM agar pywebpush bisa parsing
#     format=serialization.PrivateFormat.PKCS8,
#     encryption_algorithm=serialization.NoEncryption()
# )

# # Hati-hati: Jangan ubah ke base64 lagi untuk digunakan di pywebpush
# # Gunakan langsung dalam bentuk string PEM
# private_key_pem = private_key_bytes.decode('utf-8')

# print("✅ Public Key (Browser):", public_key_b64)
# print("✅ Private Key (Server):", private_key_pem)
...
def send_push_to_all(title, body, url="/"):
    ...
    for sub in subscriptions:
        try:
            ...
            print(f"❌ Push to {sub['endpoint']} failed:", e)
            

@app.route('/subscribe', methods=['POST'])
def subscribe():
    subscription = request.get_json()
    if not db.subscriptions.find_one({'endpoint': subscription['endpoint']}):
...
# Opsional: Favicon (untuk mencegah error 404 di console)
@app.route('/favicon.ico')
def favicon():
    return send_from_directory('static/misc', 'logo.svg', mimetype='image/svg+xml')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            send_push_to_all(
                ...
                body=f"{judul} telah dibuat di konsultasi.",
                url="/forum"
            )
...
def riwayat_pesanan():
    ...
    try:
        ...
        for transaction in transactions:
            ...
            product = db.products.find_one({'_id': ObjectId(transaction['product_id'])})
            if product is not None:
                transaction['nama_produk'] = product['nama_produk']
            else:
                transaction['nama_produk'] = transaction['product_id']  # Retain the original product ID if not found
...
def admin_transaksi():
    ...
    for transaction in transactions:
        ...
        if product is not None:
            ...
            transaction['nama_produk'] = transaction['product_id'] 
